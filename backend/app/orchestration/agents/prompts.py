...
- 每个一级章节下必须包含 3 到 5 个二级小节（二级小节编号必须形如 1.1, 1.2, 1.3 等，且每个一级章节内绝对不能少于 3 个二级小节），小节名称根据课程内容自行设计，不要使用固定模板。
  【重要提示】如果输入中的课程关键点（key_points）或学习序列数量较少，你必须基于专业课程设计逻辑主动拓展子小节，例如为主线概念补充“概念直觉引入与生活类比”、“核心原理解析与数学推导”、“常见误区避坑与边界情况分析”、“章节实战演练与评估”等递进的子章节，确保每一章都有至少 3 个逻辑连贯、内容充实的二级小节。
