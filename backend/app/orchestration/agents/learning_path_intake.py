...
def is_intake_cancellation_query(text: str) -> bool:
    normalized = text.strip()
    return bool(normalized) and any(marker in normalized for marker in CANCELLATION_MARKERS)
...
async def run_learning_path_intake_agent(state: OrchestrationState, llm: BaseChatModel) -> dict:
    ...
    try:
        intake = await _invoke_intake_draft(state, llm, query=query, profile=profile, existing_intake=existing_intake)
    except Exception as exc:
        logger.warning("LearningPathIntakeAgent structured output failed: %s: %s", type(exc).__name__, exc)
        return {"error": f"{LEARNING_PATH_INTAKE_RETRY_ERROR} ({type(exc).__name__}: {exc})", "hard_error": True}
...
async def _invoke_intake_draft(
    ...
    *,
    ...
    if not hasattr(llm, "with_structured_output"):
        logger.warning("LearningPathIntakeAgent LLM lacks structured output; using local draft fallback.")
    ...
    messages = [
        ...
        HumanMessage(content=_build_intake_generation_input(state, query, profile, existing_intake)),
    ]
    if hasattr(structured_llm, "ainvoke"):
        ...
        logger.warning("LearningPathIntakeAgent structured output is not invokable; using local draft fallback.")
        return _build_intake_draft(
            profile,
            query=query,
            user_modification_summary=query if is_intake_modification_query(query) else "",
    ...
    intake = _normalize_intake_draft(_raw_intake_to_dict(result), profile=profile, query=query)
...
def _normalize_intake_draft(intake: dict, *, profile: dict, query: str) -> dict:
    ...
    if is_intake_modification_query(query) and not str(intake.get("user_modification_summary", "")).strip():
...
def _draft_response_text(intake: dict) -> str:
    ...
    if intake.get("status") == "risk_pending":
        ...
        lines.append("这会导致相关课程的已生成内容或进度丢失。下一步是确认是否继续替换或删除这些课程。确认继续修改可直接回复“确认”“可以”“好的”“行”“继续”，也可以说“先不改”保留原路径。")
    else:
        lines.append("下一步是生成正式学习路径，系统会把这份草稿变成具体课程顺序、学习目标、时间安排和下一步行动。可以开始下一步吗？如果同意，可以直接回复“确认”“可以”“继续”，也可以点击“确认并生成学习路径”。如果想改，直接说“把第2门换成算法实战”“删掉第3门”或“主题改成前端”。")
...
def _check_risk_pending(state: OrchestrationState | dict, intake: dict) -> None:
    ...
    try:
        with Session(get_engine()) as db_session:
            outlines = db_session.exec(
                select(UserCourseKnowledgeOutline).where(UserCourseKnowledgeOutline.user_uid == user_id)
            ).all()
            for o in outlines:
                if o.course_name:
    ...
    if (not year_paths or not isinstance(year_paths, dict)) and user_id:
        try:
            ...
            logger.warning("Failed to query year learning paths for user %s: %s", user_id, exc)
    ...
    new_course_titles = {c.get("title", "").strip() for c in intake.get("courses", []) if isinstance(c, dict)}
    warnings = []
