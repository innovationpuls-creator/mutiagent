...
def _status_for_course(
    path_data: dict, rendered_grade_id: str, courses: list[dict], index: int
) -> str:
    ...
    current_course_id = (
        _clean_text(current.get("course_node_id")) if isinstance(current, dict) else ""
    )
    current_index = next(
        (
            item_index
    ...
    if index == current_index:
        progress_state = (
            _clean_text(current.get("progress_state"))
...
def _find_course(
    year_paths: dict[str, dict], course_node_id: str
...
def read_leaf_course(
    ...
    leaf_course = LeafCourseRead(
        ...
        grade_id=grade_id,
        course_or_chapter_theme=_clean_text(course.get("course_or_chapter_theme"))
        or course_node_id,
