...
export function getOutlineSummary(outline: CourseKnowledgeResult): string {
	return (
		cleanText(outline.personalization_summary) ||
		"课程大纲已生成，等待进一步补充主线说明。"
	);
}
...
export function getSectionDescription(section: CourseKnowledgeSection): string {
	return (
		...
		"围绕当前章节安排学习推进、实践验证与结果检查。"
	);
}

export function getReadableLearningSequence(
	outline: CourseKnowledgeResult,
): string[] {
	const orderedSections = getOrderedSections(outline);
	const topLevelSections = getChildSections(orderedSections, null);
	const sectionMap = new Map(
		orderedSections.map((section) => [section.section_id, section]),
	);
	...
	const allItemsAreIds = normalizedSequence.every((item) =>
		sectionMap.has(item),
	);
	if (allItemsAreIds) {
