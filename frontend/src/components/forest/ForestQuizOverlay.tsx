...
import type { ForestAttempt, CanopyOverview, ChapterWeaknessData } from '../../types/forest';
...
function GrowthTreeSVG({ stage }: { stage: number }) {
  ...
  return (
    ...
    >
      ...
      {stage === 1 && (
        <>
          <path d="M100,135 C90,128 82,130 85,138 C90,138 96,136 100,135" fill={leafColor} />
          <path d="M100,135 C110,128 118,130 115,138 C110,138 104,136 100,135" fill={leafColor} />
        </>
      )}
      ...
      {stage >= 2 && (
        <>
          ...
          {stage === 2 && (
            <>
              {/* Lower leaves */}
              <path d="M99,145 C90,140 85,142 88,148 C92,148 96,146 99,145" fill={leafColor} />
              <path d="M101,138 C110,133 115,135 112,141 C108,141 104,139 101,138" fill={leafColor} />
              {/* Top sprout */}
              <path d="M100,105 C92,98 86,102 90,108 C94,108 98,106 100,105" fill={leafColor} />
              <path d="M100,105 C108,98 114,102 110,108 C106,108 102,106 100,105" fill={leafColor} />
            </>
          )}
        </>
      )}
      ...
      {stage >= 3 && (
        <>
          ...
          {stage === 3 && (
            <>
              {/* Left Branch Leaves */}
              <path d="M80,102 C72,96 68,100 72,106 C76,106 78,104 80,102" fill={leafColor} />
              <path d="M80,102 C85,94 90,96 87,102 C84,102 82,102 80,102" fill={leafColor} />
              {/* Right Branch Leaves */}
              <path d="M122,96 C130,90 134,94 130,100 C126,100 124,98 122,96" fill={leafColor} />
              {/* Top Leaves */}
              <path d="M95,80 C87,73 83,77 87,83 C91,83 93,81 95,80" fill={leafColor} />
              <path d="M95,80 C103,73 107,77 103,83 C99,83 97,81 95,80" fill={leafColor} />
            </>
          )}
        </>
      )}
      ...
      >
        ...
        <header className="forest-overlay-header">
          ...
          <p className="forest-overlay-status-desc">
            {passed ? '恭喜通关！本次测验表现优异。' : '未达到通关分数，AI 已记录薄弱方向。'}
          </p>
        </header>
          ...
          <div className="forest-overlay-stats-section">
              ...
              <div className="stats-row">
                <div className="stat-item">
                  ...
                  <span className="stat-value">
                    {canopyOverview.completed_courses} / {canopyOverview.total_courses}
                  </span>
                </div>
                <div className="stat-item">
                  <span className="stat-label">通关章节</span>
                  <span className="stat-value">
                    {canopyOverview.completed_chapters} / {canopyOverview.total_chapters}
                  </span>
                </div>
              </div>
              <div className="stats-row" style={{ marginTop: 'var(--space-12)' }}>
                <div className="stat-item">
                  <span className="stat-label">测验均分</span>
                  <span className="stat-value">{Math.round(canopyOverview.avg_score)} 分</span>
                </div>
                <div className="stat-item">
                  <span className="stat-label">专注时长</span>
                  <span className="stat-value">{canopyOverview.total_focus_hours} 小时</span>
                </div>
              </div>
            </div>
            ...
            {weaknesses.length > 0 && (
              <div className="forest-weaknesses-card">
                <h3>// 薄弱方向收录</h3>
                <p>AI 已根据你本次错题定位以下知识点，并将在后续教学资源中自动侧重：</p>
