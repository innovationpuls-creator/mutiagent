...
export function HandwritingCanvas({ onSave, onClose }: HandwritingCanvasProps) {
  ...
  useEffect(() => {
    ...
    const baseColor = getComputedStyle(canvas).getPropertyValue('--color-text-primary').trim();
  ...
  const backdropTransition = shouldReduceMotion ? { duration: 0.12 } : motionTokens.lazy;
  const modalTransition = shouldReduceMotion ? { duration: 0.12 } : motionTokens.editorial;
      ...
      >
        ...
        <div className="flex justify-between items-center gap-[var(--space-16)]">
          <div className="flex items-center gap-[var(--space-8)]">
            ...
            />
            <span className="text-xs text-[var(--color-text-muted)] w-6 text-right">{lineWidth}px</span>
          </div>
