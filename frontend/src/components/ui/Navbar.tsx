            ...
            <AnimatePresence>
              {isDropdownOpen && (
                <motion.div
                  ...
                  role="menu"
                  aria-label="个人菜单"
                  variants={reduceMotion ? undefined : dropdownContainerVariants}
                  initial={reduceMotion ? { opacity: 0 } : 'hidden'}
                  animate={reduceMotion ? { opacity: 1 } : 'visible'}
                  exit={reduceMotion ? { opacity: 0 } : 'exit'}
                  transition={reduceMotion ? { duration: DURATION_INSTANT } : undefined}
                >
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2>期待再见</h2>
        <p>{auth.user?.username ?? '你'}，思绪已经安放，随时回来继续你的学习旅程。</p>
      </motion.div>
    </motion.div>
  );
}
