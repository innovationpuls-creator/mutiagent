...
export function ProfileCard({
	...
	return (
		<motion.div
			...
			className={`profile-card ${onClick ? "profile-card--interactive" : ""}`}
			onClick={onClick}
		...
		>
			...
			<div className="profile-body">
				<h3 className="profile-summary-title">看见自己，</h3>
				<p className="profile-summary-subtitle">才能走得更远</p>

				<p className="profile-summary-text">{summaryText}</p>
				...
				<div className="profile-tags">
					...
					<span className="profile-tag-role">
						{profile.currentGrade} · {profile.learningStage}
					</span>
					{topTags.map((tag) => (
						<span className="profile-tag-pill" key={tag}>
							{tag}
						</span>
					))}
				</div>
