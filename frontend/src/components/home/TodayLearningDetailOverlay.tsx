		...
		<div className="today-detail-outline-children">
			{children.map((section) => (
				<div className="today-detail-outline-child" key={section.section_id}>
					...
					{section.key_knowledge_points.length > 0 &&
						renderList(section.key_knowledge_points)}
...
export function TodayLearningDetailOverlay({
	...
	if (!isOpen || !data.currentCourseDetail || !data.currentLearningCourse)
		return null;
		...
		<div className="today-detail-portal">
			...
			>
				...
				<section className="today-detail-hero">
					...
					{current.next_action && (
						<p className="next-action-tip">{current.next_action}</p>
					)}
				</section>
				...
				<section className="today-detail-grid">
					...
					<div className="today-detail-section">
						<h3>时间安排</h3>
						<p>
							{course.time_arrangement.semester_scope} ·{" "}
							{course.time_arrangement.duration}
						</p>
						...
						<>
							<div className="today-detail-section">
								...
								<p className="sub-text">
									预计总投入：{getOutlineHours(data.currentCourseOutline)}
								</p>
							</div>
							...
							<div className="today-detail-section today-detail-section--wide">
								...
								<div className="today-detail-outline-stack">
									{topLevelSections.map((section) => (
										<article
											className="today-detail-outline-card"
						...
						<ol className="today-detail-grade-courses">
							{data.gradeCourses.map((item) => (
								<li
									...
									className={
										item.course_node_id === current.course_node_id
											? "is-current"
											: undefined
									}
								>
					...
					{data.followingCourses.length === 0 ? (
						...
						<ol>
							{data.followingCourses.map((item) => (
								<li key={item.course_node_id}>
									{item.course_or_chapter_theme}
								</li>
							))}
						</ol>
					)}
