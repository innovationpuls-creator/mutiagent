...
export function TodayLearningCard({ data, onClick, onStartLearning }: TodayLearningCardProps) {
  ...
  return (
    ...
    >
      ...
      <div>
        <h3 className="today-card-title">{data.currentLearningCourse?.course_or_chapter_theme ?? data.title}</h3>
        <p className="today-card-desc">{data.currentLearningCourse?.current_focus ?? data.description}</p>
        {outlineSectionCount > 0 && (
        ...
        {data.currentCourseOutline && (
          <section className="today-card-outline-preview" aria-label="课程大纲主线">
            ...
            <div className="today-card-outline-list">
              {topLevelSections.map((section) => (
                <article className="today-card-outline-item" key={section.section_id}>
