...
const FIELD_GROUPS: { title: string; keys: FieldKey[] }[] = [
	{
		title: "基础信息",
		keys: ["current_grade", "major", "learning_stage", "has_clear_goal"],
	},
	{
		title: "学习偏好",
		keys: [
	...
	{
		title: "能力基础",
		keys: ["knowledge_foundation", "strengths", "weaknesses", "experience"],
	},
	{
		title: "目标与约束",
		keys: [
...
function fieldValue(
	info: Partial<ConfirmedInfo> | undefined,
	key: FieldKey,
): string {
...
function extractQuestions(message: SessionMessage): string[] {
	if (message.question_mode === "question_box") {
		return message.question_box?.question
			? [message.question_box.question]
			: [];
	}

	const source = message.question_md || message.text;
	const section =
		source.split("❓ 接下来需要了解：")[1] ??
		source.split("接下来需要了解：")[1] ??
		"";
	const lines = section
		.split("\n")
...
function normalizeQuestionOption(
	option: QuestionBoxOption | string,
): QuestionBoxOption {
...
export function ChatCard({
	...
	const options = Array.isArray(message.question_box?.options)
		? (message.question_box.options as Array<QuestionBoxOption | string>).map(
				normalizeQuestionOption,
			)
		: [];
	const questions = extractQuestions(message);
	const generatedSections =
		message.type === "basic_profile" ? splitProfileText(message.text) : [];
	const profileSummary =
		generatedSections[0]?.body ||
		message.text ||
	...
	const isFieldStreaming = (fieldName: string): boolean => {
		if (
			...
			typeof partialData.partialData !== "object"
		)
			return false;
	...
	const isSubmitDisabled =
		...
		message.question_form.questions.some((q) => {
			...
			if (q.input_type === "multi_choice") {
				...
				if (
					arr.includes("__free_text__") &&
					!otherInputValues[q.field_name]?.trim()
				)
					return true;
			} else if (q.input_type === "single_choice") {
				if (!val) return true;
				if (val === "__free_text__" && !otherInputValues[q.field_name]?.trim())
					return true;
			} else {
							...
							<div key={q.field_name} className="form-question-group">
								...
								{(isFreeText || isOtherSelected) && (
									<input
										...
										onChange={(e) =>
											handleOtherInputChange(q.field_name, e.target.value)
										}
		...
		<CardWrapper data-state={message.type}>
			<header className="card-head">
				...
				<span className="card-state">
					{message.type === "basic_profile" ? "已生成" : "采集中"}
				</span>
			</header>
										...
										<dl>
											{fields.map(([key, value]) => (
												<div
													key={key}
													data-streaming={
														isFieldStreaming(key) ? true : undefined
													}
												>
				...
				<>
							...
							<div className="confirmed-list">
								{confirmed.map(([key, value]) => (
									<span
										className="info-chip"
										key={key}
										data-streaming={isFieldStreaming(key) ? true : undefined}
									>
					...
					{defaultedFields.length > 0 && (
						<p className="defaulted-note">
							系统已根据上下文补全：
							{defaultedFields
								.map((key) => FIELD_LABELS[key as FieldKey] ?? key)
								.join("、")}
						</p>
					)}
