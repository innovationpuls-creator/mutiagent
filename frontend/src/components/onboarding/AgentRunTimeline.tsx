...
function getCollapsedLabel(steps: AgentRunStep[]): string {
	...
	return representativeStep
		? formatStepTitle(representativeStep)
		: "多智能体流程";
}
...
export function AgentRunTimeline({
	...
	const timelineSteps = useMemo(
		() =>
			steps.map((step) => {
				if (step.status !== "running" || typeof step.startedAtMs !== "number")
					return step;
				return {
					...step,
	...
	const timingBounds = timelineSteps.reduce(
		(bounds, step) => {
			if (
				...
				!Number.isFinite(step.startedAtMs)
			) {
				return bounds;
			}
			const durationMs =
			...
			return {
				earliest:
					bounds.earliest === null
						? step.startedAtMs
						: Math.min(bounds.earliest, step.startedAtMs),
				latest:
					bounds.latest === null
						? stepEndMs
						: Math.max(bounds.latest, stepEndMs),
			};
		},
		{ earliest: null as number | null, latest: null as number | null },
	);
	const totalMs =
		timingBounds.earliest !== null && timingBounds.latest !== null
	...
	const currentStep = [...timelineSteps]
		.reverse()
					...
					<motion.div
						...
						initial={
							prefersReducedMotion
								? { opacity: 0 }
								: { opacity: 0, y: "calc(var(--space-4) * -1)" }
						}
						animate={{ opacity: 1, y: 0 }}
						exit={
							prefersReducedMotion
								? { opacity: 0 }
								: { opacity: 0, y: "var(--space-4)" }
						}
						transition={
							prefersReducedMotion
								? { duration: 0.12 }
