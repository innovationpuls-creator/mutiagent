...
import {
	...
	initialChatStore,
	nextMessageId,
} from "../../onboarding/chatReducer";
import {
	type SessionRecoveryMeta,
	useChatSession,
} from "../../onboarding/hooks/useChatSession";
import {
...
} from "../../pages/leaf/leafGenerationEvents";
import type {
	AgentRunStep,
	AgentRunStepKind,
	ChatMessage,
...
function isLoadedDataUpdate(updateType: string | undefined): boolean {
	return (
		updateType === "learning_path_loaded" ||
		updateType === "course_knowledge_loaded"
	);
}

function getDataUpdateLabel(
...
function getVisibleDuration(startTime: number, endTime: number): number {
	if (!Number.isFinite(startTime) || !Number.isFinite(endTime))
		return MIN_VISIBLE_DURATION_MS;
...
export function getSessionEventStartTime(
	...
	return (
		starts[getSessionEventTimingKey(event)] ??
		starts[getSessionEventStepId(event)] ??
		fallback
	);
}
...
function getTimelineSummary(
	...
	if (event.dependsOn && event.dependsOn.length > 0)
		details.push(`依赖：${event.dependsOn.join("、")}`);
	return details.join(" · ");
}
...
function mergeSessionAgentStep(
	current: AgentStepStatus[],
	event: SessionAgentEvent,
): AgentStepStatus[] {
...
function currentProgressLabel(steps: AgentStepStatus[]): string {
	const active = steps.find(
		(step) => step.status === "running" || step.status === "routed",
	);
	...
	return (
		...
		>
			{message.runTrace && message.runTrace.length > 0 && (
				<AgentRunTimeline steps={message.runTrace} status={message.status} />
			)}
			{children}
		</div>
	);
		...
		switch (stage) {
			...
			case "learning_preference":
				return !!(
					...
					(confirmedInfo.content_preference &&
						confirmedInfo.content_preference.length > 0) ||
...
export function AiGreetingInput({
	...
	const [agentSteps, setAgentSteps] =
		useState<AgentStepStatus[]>(DEFAULT_AGENT_STEPS);
	...
	const shouldShowPathActions = Boolean(
		latestLearningPath &&
			latestLearningPathMessageId !== hiddenPathActionsMessageId,
	);
	const hasLearningOutput = hasLearningOutputInMessages(store.messages);
	const isLearningPathStage =
		hasLearningOutput ||
		store.messages.some(
			(m) =>
			...
			(messages, sessionId) => {
				...
				hasCompleteProfileRef.current =
					...
					messages.some(
						(m) =>
							m.role === "assistant" &&
							hasCompleteBasicProfileSessionMessage(m.sessionMessage),
					);
	...
	const clearInvalidSessionAnchor = useCallback(
	...
	const expandedCardVariant =
		expandedLayout === "docked"
			? {
	...
	useEffect(() => {
		const handleGlobalMouseMove = (event: MouseEvent) => {
			...
			const xPct = Math.max(
				-0.5,
				Math.min(0.5, (event.clientX - cardCenterX) / window.innerWidth),
			);
			const yPct = Math.max(
				-0.5,
				Math.min(0.5, (event.clientY - cardCenterY) / window.innerHeight),
			);
			x.set(xPct);
			y.set(yPct);
		};
	...
	const eventToStep = useCallback(
		(event: SessionAgentEvent, now: number): { step: AgentRunStep | null } => {
			const agent = getSessionEventAgent(event);
			const label = getSessionEventTitle(event);
			const kind: AgentRunStepKind =
				(event.kind as AgentRunStepKind) || "agent";
			...
			if (event.event === "agent_calling" || event.event === "agent_progress") {
				...
				return {
					step: {
						...
						agent: agent ?? null,
						startedAtMs: getSessionEventStartTime(
			...
			if (event.event === "agent_result") {
				...
				const fallbackSummary =
					event.success === false ? "这一步没有正常完成" : "完成";
				return {
					step: {
						stepId,
						kind,
						...
						summary: getTimelineSummary(
							event,
							event.summary ?? event.error ?? event.message ?? fallbackSummary,
						),
						agent: agent ?? null,
			...
			if (event.event === "error") {
				...
				return {
					step: {
						...
						summary: getTimelineSummary(
							event,
							event.error ?? event.message ?? "这一步没有正常完成",
						),
						agent: agent ?? null,
						startedAtMs: startTime,
						durationMs: getVisibleDuration(startTime, now),
	...
	const sendMessage = useCallback(
		async (text: string, imageAttachmentArg?: string | null) => {
			...
			try {
				...
				const turn = await streamSession(
					...
					(event) => {
						...
						if (event.event === "session_started") {
							...
							executionIdRef.current =
								event.session_id ?? executionIdRef.current;
							if (event.session_id) {
								dispatch({
									type: "SET_SESSION_ID",
						...
						if (event.event === "message_completed" && event.full_text) {
							...
							if (
								isCourseKnowledgeStructuredCompletionContent(event.full_text)
							) {
								loadedCourseOutlineThisTurn = true;
							}
							return;
						...
						if (event.event === "error") {
							...
							const message =
								event.error ?? event.message ?? "对话请求失败，请稍后重试";
							dispatch({
								type: "RUN_ERROR",
								messageId: assistantMsgId,
								message,
				...
				const shouldFetchLearningPath =
					generatedLearningPathThisTurn || loadedLearningPathThisTurn;
				const shouldFetchCourseOutline =
					generatedCourseOutlineThisTurn || loadedCourseOutlineThisTurn;
				...
				if (
					...
					try {
						const structuredData = await fetchSessionState(
							token,
							finalSessionId,
						);
						if (runIdRef.current !== runId) return;
						dispatch({
							...
							sessionMessage: shouldFetchProfile
								? structuredData.profile
								: null,
							agentAnswer: null,
							learningPath: shouldFetchLearningPath
								? structuredData.learningPath
								: null,
							courseKnowledge: shouldFetchCourseOutline
						...
						if (
							shouldFetchCourseOutline &&
							structuredData.courseKnowledge?.course_id
						) {
							dispatchLeafGenerationCompleted(
								structuredData.courseKnowledge.course_id,
								"course_outline",
				...
				const errorSessionId =
					err instanceof SessionStreamError
	...
	function renderMessage(message: ChatMessage, isLatestInteractive: boolean) {
		...
		if (message.role === "assistant") {
			...
			if (message.learningPath && message.courseKnowledge) {
				...
				isStructuredMessage(message.sessionMessage)
			) {
				assistantContent = (
					<ChatCard
				...
				assistantContent = (
					<AssistantMessage
						...
						onSendReply={isLatestInteractive ? sendMessage : undefined}
						onRetryLearningPath={
							message.retryAction === "retry_learning_path"
								? retryLearningPath
								: undefined
						}
						disabled={!isLatestInteractive || isPending}
			...
			>
				{widgetState === "EXPANDED" ? (
					<section
						...
						aria-label={
							isLearningPathStage ? "AI 学习路径对话" : "AI 基础画像对话"
						}
					>
						<header className="session-header">
							<div className="session-title-cluster">
								...
								<div className="session-title-copy">
									<span>
										{isLearningPathStage ? "学习路径对话" : "基础画像对话"}
									</span>
						...
						<div className="session-workbench">
							<main className="chat-column" aria-label="对话内容">
								...
								<div className="composer-container">
									...
									!inputValue.trim() &&
									!imageAttachment &&
									!(pendingMessage && pendingMessage.mode === "draft") ? (
										<div className="composer-completed-cta-panel">
											<button
												className="cta-completed-btn"
											...
											if (activeSteps.length === 0) {
												const label = isPending
													...
													: agentSteps.some((s) => s.status === "completed")
														...
														: "等待本轮调用开始...";
												return (
													<p className="agent-step-placeholder">{label}</p>
												);
											}
											return (
