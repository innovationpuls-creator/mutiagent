...
function pickNodeStatus(
	steps: AgentRunStep[],
): AgentCollaborationNode["status"] {
...
function latestSummary(steps: AgentRunStep[], fallback: string): string {
	const latest = [...steps]
		.reverse()
		.find((step) => step.summary && step.summary.trim());
	return latest?.summary?.trim() ?? fallback;
}

export function buildAgentCollaborationNodes(
	steps: AgentRunStep[],
	...
	const orderedAgents = [
		...
		...activeAgents
			.filter(
				(agent) => !AGENT_ORDER.includes(agent as (typeof AGENT_ORDER)[number]),
			)
			.sort(),
	];
	...
	return orderedAgents.map((agent) => {
		...
		const dependsOn = [
			...new Set(agentSteps.flatMap((step) => step.dependsOn ?? [])),
		];
		const parallelGroup =
			[...agentSteps].reverse().find((step) => step.parallelGroup)
				?.parallelGroup ?? null;
		const durationMs = agentSteps.reduce(
