...
function WidgetControls() {
	...
	return (
		<div>
			...
			<button type="button" onClick={() => setWidgetState("CENTER_INPUT")}>
				center
			</button>
...
test("docks the expanded sprout recovery panel to the right edge", async () => {
	window.history.replaceState(
		{},
		"",
		"/sprout?session_id=session-recover-docked",
	);
	stubLoggedInAuth({
	...
	expect(
		screen
			.getByTestId("global-ai-widget-frame")
			.getAttribute("data-expanded-layout"),
	).toBe("docked");
	expect(
		(screen.getByTestId("global-ai-widget-shell") as HTMLDivElement).style
			.justifyContent,
	).toBe("flex-end");
});
