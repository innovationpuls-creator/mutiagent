...
vi.mock("framer-motion", () => {
	...
	return {
		...
		motion: {
			...
			button:
				createMotionTag<React.ButtonHTMLAttributes<HTMLButtonElement>>(
					"button",
				),
		},
...
test("renders text phases and triggers completion callback on button click", () => {
	...
	expect(
		screen.queryByRole("button", { name: "开始《AI Agent 开发基础能力搭建》" }),
	).toBeNull();
	...
	expect(openWithDraftMock).toHaveBeenCalledWith(
		expect.stringContaining("帮我生成《AI Agent 开发基础能力搭建》的课程大纲"),
	);
	expect(openWithDraftMock).toHaveBeenCalledWith(
