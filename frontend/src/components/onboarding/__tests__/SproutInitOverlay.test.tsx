...
test("uses opacity and transform motion props instead of filter-based animation", () => {
	...
	expect(rootOverlay?.getAttribute("data-initial")).toBe(
		JSON.stringify({ opacity: 0 }),
	);
	expect(rootOverlay?.getAttribute("data-animate")).toBe(
		JSON.stringify({ opacity: 1 }),
	);
	expect(rootOverlay?.getAttribute("data-exit")).toBe(
		JSON.stringify({ opacity: 0 }),
	);
	expect(rootOverlay?.style.backdropFilter).toBe("blur(56px)");

	expect(introHeading.getAttribute("data-initial")).toBe(
		JSON.stringify({ opacity: 0, y: 8 }),
	);
	expect(introHeading.getAttribute("data-animate")).toBe(
