...
test("clears the composer after a message is submitted", async () => {
	...
	expect(
		(screen.getByPlaceholderText("输入你的学习情况...") as HTMLTextAreaElement)
			.value,
	).toBe("");
...
test("keeps the agent timeline when a final profile answer is rendered", async () => {
	...
	await waitFor(
		() => {
			expect(
				screen.getAllByText("基础画像智能体已完成本轮处理。").length,
			).toBeGreaterThan(0);
		},
		{ timeout: 10000 },
	);
...
test("renders collecting profile card and keeps composer in incomplete-profile mode", async () => {
	...
	fireEvent.change(input, {
		target: { value: "我现在大三，你看看我的个人画像，你推荐什么？" },
	});
	fireEvent.click(screen.getByLabelText("发送消息"));
	...
	await waitFor(
		() => {
			expect(
				screen.getByText("为了生成基础画像，请先告诉我你的专业。"),
			).toBeTruthy();
...
test("recovers a persisted basic_profile card from local session cache after refresh", async () => {
	...
	window.history.replaceState(
		{},
		"",
		"/sprout?session_id=session-recover-profile",
	);
	...
	expect(
		document.querySelector(".composer-completed-cta-panel .cta-completed-btn"),
	).toBeTruthy();
});
...
test("recovers a generated basic_profile card from the server when local cache is missing", async () => {
	...
	window.history.replaceState(
		{},
		"",
		"/sprout?session_id=session-recover-profile-remote",
	);
...
test("keeps free-text composer visible after learning path intake draft is shown", async () => {
		...
		"session-session-recover-path-intake": JSON.stringify({
			...
			messages: [
				...
				},
				{
					...
					role: "assistant",
					content:
						"基础画像已经完成，我先把正式学习路径生成前的课程草稿整理出来。课程草稿：1. 数据结构入门。可以回复“确认”，也可以告诉我想修改哪一门。",
					status: "completed",
					timestamp: 1002,
					runTrace: [
						{
	...
	window.history.replaceState(
		{},
		"",
		"/sprout?session_id=session-recover-path-intake",
	);
...
test("keeps completed-profile composer mode when a cached path-only session stores hasCompleteProfile", async () => {
		...
		"session-session-recover-path-complete": JSON.stringify({
			...
			messages: [
				...
				},
				{
					...
					role: "assistant",
					content:
						"学习路径已生成，当前建议先学习《AI Agent 开发基础能力搭建》。",
					status: "completed",
					timestamp: 1001,
	...
	window.history.replaceState(
		{},
		"",
		"/sprout?session_id=session-recover-path-complete",
	);
...
test("keeps completed-profile composer mode when the server recovers an outline-only session", async () => {
	...
	const fetchMock = vi.fn().mockResolvedValue({
		...
		json: async () => ({
			...
			messages: [
				...
				{
					type: "ai",
					data: { content: "课程大纲已生成：《AI Agent 开发基础能力搭建》。" },
				},
			],
			profile: {
				type: "basic_profile",
	...
	window.history.replaceState(
		{},
		"",
		"/sprout?session_id=session-recover-outline-complete",
	);
...
test("recovers a persisted collecting profile card without marking the profile as completed", async () => {
	...
	window.history.replaceState(
		{},
		"",
		"/sprout?session_id=session-recover-collecting",
	);
	...
	expect(screen.getByPlaceholderText("输入你的学习情况...")).toBeTruthy();
	expect(
		screen.queryByPlaceholderText("画像已生成，可以继续补充或追问..."),
	).toBeNull();
});
...
test("renders worker timeline even when stream skips supervisor_plan events", async () => {
	...
	await waitFor(
		() => {
			expect(
				screen.getAllByText("课程大纲智能体结果已生成").length,
			).toBeGreaterThan(0);
		},
		{ timeout: 10000 },
	);
...
test("persists a failed session so refresh can recover the retry entrypoint", async () => {
	...
	expect(persisted.messages.some((message) => message.status === "error")).toBe(
		true,
	);
	expect(
		persisted.messages.some(
			(message) => message.retryAction === "retry_learning_path",
		),
	).toBe(true);
	...
	window.history.replaceState(
		{},
		"",
		"/sprout?session_id=session-failed-refresh",
	);
	...
	await waitFor(() => {
		expect(screen.getAllByText("学习路径生成失败").length).toBeGreaterThan(0);
		expect(
			screen.getByRole("button", { name: "重试生成学习路径" }),
		).toBeTruthy();
	});
});
...
test("updates the progress panel as soon as streaming agent events arrive", async () => {
	...
	await waitFor(() => {
		expect(
			screen.queryByText(/Unexpected non-whitespace character after JSON/),
		).toBeNull();
		expect(screen.getAllByText("记忆智能体").length).toBeGreaterThan(0);
		expect(screen.getAllByText("正在读取历史对话记录").length).toBeGreaterThan(
			0,
		);
		expect(screen.getAllByText("正在提取用户画像数据").length).toBeGreaterThan(
			0,
		);
		expect(screen.getAllByText("正在提取学习路径数据").length).toBeGreaterThan(
			0,
		);
		expect(screen.getAllByText("正在提取课程大纲数据").length).toBeGreaterThan(
			0,
		);
		expect(screen.getAllByText("意图识别智能体").length).toBeGreaterThan(0);
		expect(
			screen.getAllByText("正在判断本轮要调用的智能体").length,
		).toBeGreaterThan(0);
		expect(screen.getAllByText("基础画像智能体").length).toBeGreaterThan(0);
		expect(
			screen.getAllByText("基础画像智能体开始处理。").length,
		).toBeGreaterThan(0);
	...
	await waitFor(() => {
		expect(screen.getByText("基础画像已完成")).toBeTruthy();
		expect(screen.getAllByText("本轮智能体调用已完成").length).toBeGreaterThan(
			0,
		);
	});
});
...
test("auto-sends the pending start-course message when the widget opens from sprout", async () => {
	...
	await waitFor(() => {
		...
		expect(
			fetchMock.mock.calls.some(
				([url]) =>
					url === "http://127.0.0.1:8000/api/chat/sessions/session-course",
			),
		).toBe(true);
		expect(screen.getByText("AI Agent 开发基础能力搭建")).toBeTruthy();
		expect(
			screen.getByText("先完成需求拆解，再进入接口接入与最小闭环演示。"),
		).toBeTruthy();
...
test("renders both learning path and course outline when the same turn stores both structured results", async () => {
	...
	await waitFor(() => {
		expect(
			screen.getByText("学习路径 · learning_path.v2.course_node"),
		).toBeTruthy();
		expect(screen.getByText("课程大纲 · year_3")).toBeTruthy();
	});
});
...
test("reuses the same session after profile completion instead of creating a new session", async () => {
	...
	window.addEventListener(
		"mutiagent-learning-path-updated",
		learningPathUpdated,
	);
	...
	expect(mockNavigate).toHaveBeenCalledWith("/branch", {
		state: { justGeneratedProfile: true },
	});
	...
	await waitFor(() => {
		expect(
			fetchMock.mock.calls.some(
				([url]) =>
					url ===
					"http://127.0.0.1:8000/api/chat/sessions/session-outline-loaded",
			),
		).toBe(true);
		expect(screen.getByText("课程大纲 · year_3")).toBeTruthy();
		expect(screen.getByText("AI Agent 开发基础能力搭建")).toBeTruthy();
	...
	await waitFor(() => {
		expect(
			fetchMock.mock.calls.some(
				([url]) =>
					url ===
					"http://127.0.0.1:8000/api/chat/sessions/session-path-fallback-text",
			),
		).toBe(true);
		expect(screen.getByText(/课程路径/)).toBeTruthy();
		expect(screen.getByText("AI Agent 开发基础能力搭建")).toBeTruthy();
	...
	await waitFor(() => {
		expect(
			fetchMock.mock.calls.some(
				([url]) =>
					url ===
					"http://127.0.0.1:8000/api/chat/sessions/session-outline-fallback-text",
			),
		).toBe(true);
		expect(screen.getByText("课程大纲 · year_3")).toBeTruthy();
		expect(screen.getByText("AI Agent 开发基础能力搭建")).toBeTruthy();
...
test("allows using handwriting canvas, previews sketch, and submits message with image attachment", async () => {
	...
	await waitFor(() => {
		...
		expect(lastFetchBody.image_attachment).toBe(
			"data:image/png;base64,fake-data",
		);
	...
	await waitFor(() => {
		...
		expect(bubbleImg.getAttribute("src")).toBe(
			"data:image/png;base64,fake-data",
		);
...
it("renders progress bar indicating the active collection stage", () => {
	...
	expect(
		screen.getByText(/欢迎！告诉我你的年级、专业、学习目标或当前卡点/),
	).toBeTruthy();
