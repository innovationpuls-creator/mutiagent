...
const CENTER_INPUT_OFFSET =
	"min(25vh, calc(var(--space-120) + var(--space-96)))";
...
function hasPendingSproutInitOverlay(): boolean {
	...
	try {
		return (
			window.sessionStorage.getItem(SPROUT_INIT_OVERLAY_KEY) === "1" ||
...
export function GlobalAiWidget() {
	...
	const isDockedExpanded =
		widgetState === "EXPANDED" && expandedLayout === "docked";
				...
				{widgetState === "EXPANDED" && (
					<motion.div
						...
						transition={
							reduceMotion
								? { duration: 0 }
								: { duration: 0.98, ease: [0.25, 1, 0.5, 1] }
						}
						style={{
							position: "fixed",
							inset: 0,
				...
				{widgetState !== "HIDDEN" && token && (
					<motion.div
						...
						transition={
							reduceMotion
								? { duration: 0 }
								: { duration: 0.98, ease: [0.64, 0, 0.35, 1] }
						}
						style={{
							position: "fixed",
							inset: 0,
					...
					>
						<div
							...
							style={{
								pointerEvents: "auto",
								transform:
									widgetState === "CENTER_INPUT"
										? `translateY(${CENTER_INPUT_OFFSET})`
										: "translateY(0)",
