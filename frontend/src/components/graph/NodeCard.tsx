...
import { breathingGlowVariants, actionHintVariants, whisperVariants } from '../../lib/motion';
...
export function NodeCard({ id, title, status, agentMessage, completedConcepts, totalConcepts }: NodeCardProps) {
  // Ensure we don't go below 15% width so the pill remains beautifully pebble-shaped
  const rawRatio = totalConcepts > 0 ? (completedConcepts / totalConcepts) * 100 : 0;
        ...
        <AnimatePresence mode="wait">
          {isAgentActive ? (
            <motion.div
              ...
              exit="exit"
            >
              <span className={`whisper-dot ${status === 'in-progress' ? 'active' : 'planner'}`} />
              {agentMessage}
            </motion.div>
          ) : (
            <motion.div
