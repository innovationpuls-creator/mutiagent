	...
	useEffect(() => {
		...
		const run = async () => {
			...
			while (alive) {
				...
				runAnimate(
					".node-el",
					{ opacity: 0, filter: "blur(10px)", scale: 0.88 },
					{ duration: 0 },
				);
				...
				runAnimate(
					"#pulse-line",
					{ opacity: 0, pathLength: 0, pathOffset: 0 },
					{ duration: 0 },
				);
				...
				runAnimate(
					".term-char-term-1",
					{ opacity: 1 },
					{ duration: 0.01, delay: stagger(0.018) },
				);
				...
				runAnimate(
					".task-item",
					{ opacity: 1, x: 0 },
					{ duration: 0.3, delay: stagger(0.16), ease: "easeOut" },
				);
				await wait(1250);
				...
				runAnimate(
					".check-icon",
					{ opacity: 1, scale: 1 },
					{ duration: 0.2, delay: stagger(0.2), ease: "easeOut" },
				);
				await wait(1350);
				...
				runAnimate(
					".term-char-term-4",
					{ opacity: 1 },
					{ duration: 0.01, delay: stagger(0.016) },
				);
				await wait(1555);
				await wait(1640);
				if (!alive) break;
				...
				runAnimate(
					".tree-branch",
					{ pathLength: 1 },
					{ duration: 0.35, delay: stagger(0.1), ease: "easeOut" },
				);
				await wait(500);
		...
		<div ref={scope} className="agent-sandtable">
			<div className="agent-sandtable__stage">
				...
				>
					...
					<motion.path
						id="line-0"
						className="sandtable-line hide-on-reset"
						d={line0}
						fill="none"
						...
						<div style={{ display: "flex", gap: 3, marginTop: 6 }}>
							...
							<div
								style={{
									height: 3,
									flex: 0.6,
									background: "var(--color-intent-success)",
									opacity: 0.5,
									borderRadius: 1.5,
								}}
							/>
						...
						>
							<div
								style={{
									...
									borderRadius: "50%",
									background: "var(--color-intent-success)",
									boxShadow: "0 0 6px var(--color-intent-success)",
								}}
							/>
							{s.label}
						</div>
			...
			<div className="agent-sandtable__terminal">
				...
				<div className="agent-sandtable__terminal-header">
					...
					<span className="agent-sandtable__terminal-title">运行日志</span>
				</div>
				{/* Terminal 输出区 */}
				<div className="agent-sandtable__terminal-body">
					...
					<div
						className="terminal-cursor hide-on-reset"
						style={{ opacity: 0 }}
					/>
				</div>
			</div>
