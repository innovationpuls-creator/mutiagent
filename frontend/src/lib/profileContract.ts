...
export function profileYearIdFromCurrentGrade(
	currentGrade: string,
): GradeId | null {
	...
	for (const [label, yearId] of Object.entries(
		SUPPORTED_PROFILE_GRADE_TO_YEAR_ID,
	)) {
...
export function isSupportedProfileCurrentGrade(currentGrade: unknown): boolean {
	return (
		...
		profileYearIdFromCurrentGrade(currentGrade) !== null
	);
}
...
export function hasCompleteBasicProfileRecord(
	...
	if (
		!isSupportedProfileCurrentGrade(
			(confirmedInfo as Record<string, unknown>).current_grade,
		)
	)
		return false;
	return requiredKeys.every((key) => Object.hasOwn(confirmedInfo, key));
}

export function hasCompleteBasicProfileSessionMessage(
	profile: SessionMessage | null | undefined,
): boolean {
	if (!profile || profile.type !== "basic_profile") return false;
