...
describe("TeacherPage State Machine & localStorage Saves", () => {
	...
	it("advances empty -> loading -> editor on valid file drop", async () => {
		...
		expect(
			screen.getByText("管理员：测试教师 | 正在编辑已对齐的人培课程方案"),
		).toBeTruthy();
	...
	it("shows editable publishing scope with completion hints from existing programs", async () => {
		...
		expect(
			screen.getByText("必须和学生账号里的 school 完全一致。"),
		).toBeTruthy();
		expect(
			screen.getByText("选定学校后，会优先提示该学校已有专业。"),
		).toBeTruthy();
		expect(
			screen.getByText("发布后只会匹配同一学校、专业、班级的学生。"),
		).toBeTruthy();
	...
	it("uses a dropdown option to fill the publishing scope and blocks publishing when it is incomplete", async () => {
		...
		await act(async () => {
			fireEvent.change(screen.getByDisplayValue("南山大学"), {
				target: { value: "" },
			});
		});
	...
	it("opens DetailDrawer, edits values, and publishes updates through the management API", async () => {
		...
		expect(teacherProgramApiMocks.publishTeacherProgram).toHaveBeenCalledWith(
			"test-token",
			expect.any(Array),
			{
				school: "南山大学",
		...
		const savedCourses = teacherProgramApiMocks.publishTeacherProgram.mock
			.calls[0][1] as BranchCourseNode[];
