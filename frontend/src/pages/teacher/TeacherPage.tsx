...
function ProgramScopePanel({ scope, scopeOptions, onChange }: ProgramScopePanelProps) {
  ...
  const schoolOptions = uniqueValues(scopeOptions.map((option) => option.school));
  const schoolMatchedScopes = scopeOptions.filter((option) => !currentSchool || option.school === currentSchool);
  const majorSourceScopes = schoolMatchedScopes.length > 0 ? schoolMatchedScopes : scopeOptions;
  const majorOptions = uniqueValues(
    majorSourceScopes.map((option) => option.major)
  );
  const majorMatchedScopes = majorSourceScopes.filter((option) => !currentMajor || option.major === currentMajor);
  const classSourceScopes = majorMatchedScopes.length > 0 ? majorMatchedScopes : majorSourceScopes;
  const classNameOptions = uniqueValues(classSourceScopes.map((option) => option.className));
  const scopeItems = [
    {
      key: 'school',
      label: '学校',
  ...
  return (
    <section className="program-scope-panel" aria-labelledby="program-scope-title">
      <div className="program-scope-copy">
        ...
        <p>填写时可直接选已有组织班级；发布后，学生端会用账号里的学校、专业、班级精确匹配这份人培方案。</p>
      </div>
      <div className="program-scope-summary">
            ...
            <div className="program-scope-field" key={item.key}>
              ...
              <div className="program-scope-combobox">
                ...
                <button
                  ...
                  onClick={() => setOpenField(openField === item.key ? null : item.key)}
                >
                  ▾
                </button>
                {openField === item.key ? (
                  ...
                  >
                    {item.options.length > 0 ? (
                      item.options.slice(0, 8).map((option) => (
                        <button
                          type="button"
                          className={option === item.value.trim() ? 'program-scope-list-option is-active' : 'program-scope-list-option'}
                          key={option}
                          role="option"
                      ...
                      <span className="program-scope-list-empty">暂无已有数据，可手动填写。</span>
                    )}
                  </div>
                ) : null}
              </div>
        ...
        <div className={`program-scope-status ${hasCompleteScope ? '' : 'program-scope-status-warning'}`}>
          <span>{hasCompleteScope ? '发布范围已填写，请确认和学生账号完全一致' : '学校、专业、班级都填写后才能发布'}</span>
        </div>
      </div>
    </section>
  );
...
export function TeacherPage() {
  ...
  const [programScopeOptions, setProgramScopeOptions] = useState<ProgramScope[]>([]);
  ...
  useEffect(() => {
    ...
    adminDataApi.programs(token)
      .then((programs) => {
        if (cancelled) return;
        setProgramScopeOptions(programs.map((program) => ({
      ...
      .catch(() => {
        if (!cancelled) setProgramScopeOptions([]);
      });
  ...
  const handleSave = async () => {
    ...
    if (!scope.school || !scope.major || !scope.className) {
      setToastMessage('请填写学校、专业、班级；这些值必须和学生账号完全一致');
      setTimeout(() => {
  ...
  return (
    ...
    >
      ...
      <div className="teacher-container">
        <ProgramScopePanel scope={programScope} scopeOptions={programScopeOptions} onChange={setProgramScope} />
        {courses.length > 0 ? (
