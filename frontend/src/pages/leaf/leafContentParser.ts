import type { LeafComposedSection, LeafCourseResponse, LeafSection } from '../../types/leaf';
...
function stripTopLevelTitlePrefix(title: string): string {
  ...
  if (numericPattern.test(title)) return title.replace(numericPattern, '').trim();
  return title;
}

export function getOrderedLeafSections(sections: LeafSection[]): LeafSection[] {
  return [...sections].sort((left, right) => left.order_index - right.order_index);
}

export function getLeafChildSections(sections: LeafSection[], parentSectionId: string | null): LeafSection[] {
  return getOrderedLeafSections(sections).filter((section) => section.parent_section_id === parentSectionId);
}
...
export function getLeafSectionHeading(section: LeafSection): string {
  ...
  const title = section.section_id.includes('.') ? rawTitle : stripTopLevelTitlePrefix(rawTitle);
...
export function hasLeafComposedContent(response: LeafCourseResponse, sectionId: string): boolean {
...
export function isLeafSection(sections: LeafSection[], section: LeafSection): boolean {
  return !sections.some((item) => item.parent_section_id === section.section_id);
}

export function getDefaultLeafSectionId(response: LeafCourseResponse): string | null {
  const orderedSections = getOrderedLeafSections(response.sections);
  const generatedLeafSection = orderedSections.find((section) => (
    isLeafSection(orderedSections, section) && hasLeafComposedContent(response, section.section_id)
  ));
  if (generatedLeafSection) return generatedLeafSection.section_id;

  const firstTopLevelSection = orderedSections.find((section) => section.parent_section_id === null);
  return firstTopLevelSection?.section_id ?? orderedSections[0]?.section_id ?? null;
}

export function findLeafSectionById(sections: LeafSection[], sectionId: string | null): LeafSection | null {
  if (!sectionId) return null;
...
export function getTopLevelSectionForLeaf(
  ...
  while (currentSection.parent_section_id) {
    const parentSection = findLeafSectionById(sections, currentSection.parent_section_id);
