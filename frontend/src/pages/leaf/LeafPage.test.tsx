...
vi.mock("../../context/AiWidgetContext", async () => {
	const actual = await vi.importActual<
		typeof import("../../context/AiWidgetContext")
...
describe("LeafPage", () => {
	it("loads course content and selects first generated leaf section", async () => {
		...
		expect(
			screen.queryByText("It provides a foundational overview."),
		).toBeNull();
	});
	...
	it("renders fenced code blocks without a language as block code", async () => {
		...
		expect(
			view.container.querySelector(".code-block pre code")?.textContent,
		).toContain("plain block line");
	});
	...
	it("renders the video cover when a video resource is available", async () => {
								...
								videos: [
									{
										...
										cover_url:
											"https://img.youtube.com/vi/nQX61qSL-uE/hqdefault.jpg",
		...
		const image = await screen.findByAltText(
			"深入 LangChain 系列 - PromptTemplate",
		);
		expect(image.getAttribute("src")).toBe(
			"https://img.youtube.com/vi/nQX61qSL-uE/hqdefault.jpg",
		);
		expect(
			screen.getByText("深入 LangChain 系列 - PromptTemplate"),
		).toBeTruthy();
	});
								...
								videos: [
									{
										...
										url: "https://www.youtube.com/watch?v=nQX61qSL-uE",
										cover_url:
											"https://img.youtube.com/vi/nQX61qSL-uE/missing.jpg",
										cover_status: "provided",
										source: "youtube",
									},
								],
	...
	it("opens AI draft prompt from first chapter generation entry", async () => {
		fetchLeafCourseMock.mockResolvedValue(
			leafPayload({ section_composed_markdowns: {} }),
		);

		renderLeaf();

		await waitFor(() =>
			expect(screen.getByText("让 AI 生成本章内容")).toBeTruthy(),
		);
		...
		expect(openWithDraftMock).toHaveBeenCalledWith(
			expect.stringContaining("course_node_id: year_3_course_1"),
		);
		expect(openWithDraftMock).toHaveBeenCalledWith(
			expect.stringContaining("mode: generate"),
		);
	});
	...
	it("opens AI draft prompt from the left outline generation entry", async () => {
		...
		fetchLeafCourseMock.mockResolvedValue(
			leafPayload({
				course: {
					...(base.course as Record<string, unknown>),
					has_outline: false,
				},
				outline: null,
				sections: [],
	...
	it("refreshes leaf outline after course outline generation completes and hides the outline entry", async () => {
		...
		window.dispatchEvent(
			new CustomEvent("mutiagent-leaf-generation-completed", {
		...
		const base = leafPayload({
			sections: [
				...
				},
				{
					...
					parent_section_id: null,
					depth: 1,
					title: "第三章：Vector Similarity Search Implementation",
					order_index: 3,
					description: "实现向量检索",
	...
	it("opens AI regeneration prompt when the chapter already has generated leaf content", async () => {
		...
		expect(openWithDraftMock).toHaveBeenCalledWith(
			expect.stringContaining("mode: regenerate"),
		);
	});
	...
	it("shows generation failure without reloading stale content", async () => {
		fetchLeafCourseMock.mockResolvedValue(
			leafPayload({ generation_status: null }),
		);
