  ...
  return (
    <nav className="relative z-10 flex items-center justify-between gap-4 mb-12">
      ...
      <div className="flex items-center flex-wrap gap-4 justify-end">
        {statusMessage ? (
          <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-medium border border-[var(--glass-border)] shadow-[var(--shadow-sm)] ${generationPillClass}`} role="status">
            <span className="relative flex h-2 w-2">
              {isGenerationError ? null : (
                <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${generationDotClass}`} />
              )}
              <span className={`relative inline-flex rounded-full h-2 w-2 ${generationDotClass}`} />
            </span>
          ...
          <div className="flex items-center gap-2 text-sm font-medium text-[var(--color-text-secondary)]">
            ...
            <span className="text-[var(--color-text-primary)] font-medium">{courseStatusLabel(course.status)}</span>
          </div>
        )}

        <div className="h-4 w-px bg-[var(--color-border)] opacity-60 mx-1" aria-hidden="true" />
