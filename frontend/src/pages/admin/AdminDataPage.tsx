...
import {
	type DataCohort,
	type DataOverview,
	adminDataApi as defaultAdminDataApi,
...
function matchesCohortText(
	...
	if (!text) return true;
	return (
		school.includes(text) || major.includes(text) || className.includes(text)
	);
}

function dataCount(
	data: UserLearningData | null,
	key: keyof Omit<UserLearningData, "user" | "profile">,
) {
...
export function AdminDataPage({
	...
	const [learningData, setLearningData] = useState<UserLearningData | null>(
		null,
	);
	...
	const loadData = async () => {
		...
		setError(null);
		try {
			const [nextOverview, nextCohorts, nextPrograms, nextAccounts] =
				await Promise.all([
			...
			setError(
				dataError instanceof Error ? dataError.message : "数据管理加载失败",
			);
		} finally {
			setBusy(false);
		}
	...
	useEffect(() => {
		...
		let cancelled = false;
		adminDataApi
			.userLearningData(token, selectedUid)
			.then((data) => {
			...
			.catch((dataError) => {
				if (!cancelled)
					setError(
						dataError instanceof Error ? dataError.message : "学习数据加载失败",
					);
			});
		return () => {
			cancelled = true;
	...
	const filteredCohorts = useMemo(() => {
		...
		return cohorts.filter((cohort) =>
			matchesCohortText(text, cohort.school, cohort.major, cohort.class_name),
		);
	}, [cohorts, query]);
	...
	const deleteSelectedLearningData = async () => {
		...
		try {
			...
			setError(
				dataError instanceof Error ? dataError.message : "学习数据清理失败",
			);
		} finally {
			setBusy(false);
		}
	...
	const deleteProgram = async (cohort: DataCohort) => {
		...
		try {
			...
			setError(
				dataError instanceof Error ? dataError.message : "人培方案删除失败",
			);
		} finally {
			setBusy(false);
		}
...
function OverviewPanel({ overview }: { overview: DataOverview | null }) {
	const learningCount = overview
		? Object.values(overview.learning_data).reduce(
	...
	return (
		<section className="admin-data-overview" aria-label="数据概览">
			<article>
				...
				<strong>
					{overview
						? Object.values(overview.accounts).reduce(
								(total, value) => total + value,
								0,
							)
						: 0}
			...
			<article>
				<span>组织班级</span>
				<strong>{overview?.cohorts ?? 0}</strong>
			</article>
			<article>
				<span>人培方案</span>
				<strong>{overview?.programs ?? 0}</strong>
			</article>
			<article>
				<span>学习数据</span>
		...
		<section className="admin-data-table" aria-label="组织班级列表">
			...
			{cohorts.map((cohort) => (
				<div
					...
					key={`${cohort.school}-${cohort.major}-${cohort.class_name}`}
				>
					...
					<button
						type="button"
						disabled={busy || !cohort.has_program}
						onClick={() => onDeleteProgram(cohort)}
					>
						删除方案
					</button>
				</div>
				...
				<div className="admin-data-row" key={program.program_id}>
					...
					<span>
						{program.published_at
							? new Date(program.published_at).toLocaleString("zh-CN")
							: "未发布"}
					</span>
				</div>
			))}
...
function LearningDataPanel({
	...
	return (
		<section
			className="admin-data-detail"
			aria-label={mode === "learning" ? "学习数据详情" : "会话数据详情"}
		>
			<label>
				<span>账号</span>
				<select
			...
			{selectedAccount ? (
				<div className="admin-data-user-card">
					...
					<span>
						{selectedAccount.school} / {selectedAccount.major} /{" "}
						{selectedAccount.class_name}
					</span>
				</div>
			) : null}
			{mode === "learning" ? (
				<div className="admin-data-metrics">
					<article>
						<span>画像</span>
						<strong>{learningData?.profile ? 1 : 0}</strong>
					</article>
					<article>
						<span>学习路径</span>
						<strong>{dataCount(learningData, "year_learning_paths")}</strong>
					</article>
					<article>
						<span>课程大纲</span>
						<strong>{dataCount(learningData, "course_outlines")}</strong>
					</article>
					<article>
						<span>测验</span>
						<strong>{dataCount(learningData, "chapter_quizzes")}</strong>
					</article>
					<article>
						<span>进度</span>
						<strong>{dataCount(learningData, "chapter_progress")}</strong>
					</article>
					<article>
						<span>资源质量</span>
						<strong>{dataCount(learningData, "resource_quality")}</strong>
					</article>
				</div>
			) : (
				<div className="admin-data-metrics">
					<article>
						<span>会话</span>
						<strong>{dataCount(learningData, "conversation_sessions")}</strong>
					</article>
				</div>
			)}
			<button
