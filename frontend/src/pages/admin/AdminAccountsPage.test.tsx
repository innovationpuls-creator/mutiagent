...
describe("AdminAccountsPage", () => {
	it("renders menu and manages accounts through the admin API", async () => {
		...
		const adminApi: AdminAccountApi = {
			...
			importAccounts: vi
				.fn()
				.mockResolvedValue({ created: 0, updated: 0, failed: 0, failures: [] }),
			exportAccounts: vi
				.fn()
				.mockResolvedValue("username,identifier,password,role,is_active\n"),
		};
		...
		await waitFor(() => {
			expect(adminApi.updateAccount).toHaveBeenCalledWith(
				"token-1",
				"teacher-1",
				{
					username: "王教师二号",
		...
		const enabledStatusButton = screen
		...
		await waitFor(() => {
			expect(adminApi.updateAccount).toHaveBeenLastCalledWith(
				"token-1",
				"teacher-1",
				{
					username: "王教师二号",
		...
		await waitFor(() => {
			expect(adminApi.deleteAccount).toHaveBeenCalledWith(
				"token-1",
				"teacher-1",
			);
