...
export function AdminAccountsPage({ adminApi = defaultAdminApi }: AdminAccountsPageProps) {
  ...
  useEffect(() => {
    ...
    adminApi.listAccounts(token)
      ...
      .catch((accountError) => {
        if (!cancelled) setError(accountError instanceof Error ? accountError.message : '账号加载失败');
      })
      .finally(() => {
  ...
  const selectedAccounts = useMemo(() => selectedAccountList(accounts, selectedUids), [accounts, selectedUids]);
  const visibleSelectedCount = visibleAccounts.filter((account) => selectedUids.has(account.uid)).length;
  ...
  const deleteCount = deleteTarget?.type === 'single' ? 1 : deleteTarget?.accounts.length ?? 0;
  ...
  const saveAccount = async () => {
    ...
    try {
      if (editorMode === 'edit' && editingAccount) {
        const updated = await adminApi.updateAccount(token, editingAccount.uid, {
          username,
          identifier,
          role: draft.role,
      ...
      setError(accountError instanceof Error ? accountError.message : '账号保存失败');
    } finally {
      setBusy(false);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      if (deleteTarget.type === 'single') {
        ...
        setAccounts((current) => current.filter((account) => account.uid !== deleteTarget.account.uid));
        setSelectedUids((current) => {
      ...
      if (editingAccount && deleteTarget.type === 'single' && editingAccount.uid === deleteTarget.account.uid) {
        closeEditor();
      }
    } catch (accountError) {
      setError(accountError instanceof Error ? accountError.message : '账号删除失败');
    } finally {
      setBusy(false);
    }
  };
  ...
  const toggleAccount = async (account: AuthUser) => {
    ...
    try {
      ...
      setError(accountError instanceof Error ? accountError.message : '账号状态更新失败');
    } finally {
      setBusy(false);
    }
  };
  ...
  const runBatch = async (action: AdminBatchAction, role?: AuthRole) => {
    ...
    try {
      ...
      setError(accountError instanceof Error ? accountError.message : '批量操作失败');
    } finally {
      setBusy(false);
    }
  };
  ...
  const saveResetPassword = async () => {
    ...
    try {
      ...
      setError(accountError instanceof Error ? accountError.message : '密码重置失败');
    } finally {
      setBusy(false);
    }
  };
  ...
  const importCsv = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setError(accountError instanceof Error ? accountError.message : 'CSV 导入失败');
    } finally {
      setBusy(false);
    }
  };
  ...
  const exportCsv = async () => {
    ...
    try {
      ...
      setError(accountError instanceof Error ? accountError.message : '账号导出失败');
    } finally {
      setBusy(false);
    }
  };
      ...
      <section className="admin-shell" aria-labelledby="admin-title">
        ...
        <header className="admin-header">
          ...
          <div className="admin-header-actions">
            <button className="admin-secondary-action" type="button" onClick={() => void exportCsv()} disabled={busy}>
            ...
            />
            <button className="admin-primary-action" type="button" onClick={openCreateEditor} disabled={busy}>
        ...
        <section className="admin-toolbar" aria-label="账号筛选和批量管理">
          <label className="admin-search">
            ...
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索用户名或登录标识" />
          </label>
        ...
        {selectedAccounts.length > 0 ? (
          <section className="admin-batchbar" aria-label="批量管理工具栏">
            <span className="admin-batch-count">已选 {selectedAccounts.length} 个账号</span>
            <button type="button" onClick={() => void runBatch('activate')} disabled={busy}>
              批量启用
            </button>
            <button type="button" onClick={() => void runBatch('deactivate')} disabled={busy || selectedHasSelf}>
              批量停用
            </button>
            ...
            <button className="admin-danger-action" type="button" onClick={requestBatchDelete} disabled={busy || selectedHasSelf}>
              批量删除
            </button>
            <button className="admin-clear-action" type="button" onClick={() => setSelectedUids(new Set<string>())}>
              清空选择
            </button>
          </section>
        ) : null}
        ...
        {importResult ? (
          <section className="admin-import-result" aria-label="CSV 导入结果">
            <div>
              <strong>导入完成</strong>
              <span>创建 {importResult.created} 个，更新 {importResult.updated} 个，失败 {importResult.failed} 行</span>
            </div>
            {importResult.failures.length > 0 ? (
              <ul>
                {importResult.failures.map((failure) => (
                  <li key={`${failure.row}-${failure.identifier ?? 'empty'}`}>
                    第 {failure.row} 行：{failure.identifier ?? '无登录标识'}，{failure.reason}
                  </li>
                ))}
              </ul>
            ) : null}
            <button type="button" onClick={() => setImportResult(null)} aria-label="关闭导入结果">
              <X aria-hidden="true" />
            </button>
          </section>
        ) : null}
        ...
        <section className="admin-table" aria-label="账号列表" aria-busy={busy}>
            ...
            return (
              <div className="admin-table-row" key={account.uid}>
                ...
                <span className="admin-row-actions">
                  <button type="button" onClick={() => openEditEditor(account)} aria-label={`编辑 ${account.username}`} disabled={busy}>
          ...
          {visibleAccounts.length === 0 ? (
            <p className="admin-empty">{busy ? '正在加载账号。' : '没有匹配的账号。'}</p>
          ) : null}
        </section>
      </section>
      ...
      <AnimatePresence>
        {isEditorOpen ? (
          <motion.aside
            ...
            transition={reduceMotion ? { duration: 0.12 } : { duration: 0.42, ease: [0.33, 1, 0.68, 1] }}
          >
            <header>
              <div>
            ...
            <div className="admin-drawer-form">
              ...
              <label>
                ...
                <select
                  value={draft.role}
                  onChange={(event) => setDraft((current) => ({ ...current, role: event.target.value as AuthRole }))}
            ...
            <footer>
              <button className="admin-secondary-action" type="button" onClick={closeEditor}>
                取消
              </button>
              <button className="admin-primary-action" type="button" onClick={() => void saveAccount()} disabled={busy}>
        ...
        {deleteTarget ? (
          ...
          >
            <section className="admin-modal" aria-label="确认删除账号">
              ...
              <footer>
                <button className="admin-secondary-action" type="button" onClick={() => setDeleteTarget(null)}>
                  取消
                </button>
                <button className="admin-danger-action" type="button" onClick={() => void confirmDelete()} disabled={busy}>
                  确认删除
                </button>
              </footer>
            </section>
          ...
          >
            <section className="admin-modal" aria-label="重置密码">
              ...
              <footer>
                ...
                <button className="admin-primary-action" type="button" onClick={() => void saveResetPassword()} disabled={busy}>
