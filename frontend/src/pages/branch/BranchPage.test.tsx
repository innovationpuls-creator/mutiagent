...
vi.mock("../../context/AiWidgetContext", () => ({
	...
	AiWidgetProvider: ({ children }: { children: React.ReactNode }) => (
		<>{children}</>
	),
}));
...
vi.mock("../../api/profile", () => ({
	fetchProfileDashboard: (...args: unknown[]) =>
		fetchProfileDashboardMock(...args),
}));
...
describe("BranchPage", () => {
	...
	it("uses the upper course nodes instead of rendering a lower course rail", async () => {
		...
		await waitFor(() => {
			expect(
				screen.getByText("这一学年共 3 门课程，按顺序慢慢推进。"),
			).toBeTruthy();
		});
		...
		fireEvent.click(
			screen.getByRole("button", {
				name: /大三第 1 门课程（自选课程）：AI 应用开发基础能力搭建，已完成/,
			}),
		);
	...
	it("navigates completed and current courses to leaf while locked courses stay on branch", async () => {
		...
		fireEvent.click(
			screen.getByRole("button", {
				name: /大三第 2 门课程（自选课程）：当前课程，进行中/,
			}),
		);
		await waitFor(() =>
			expect(screen.getByTestId("location").textContent).toBe(
				"/leaf/year_3_course_2",
			),
		);
		currentView.unmount();
	...
	it("renders PathInitOverlay when justGeneratedProfile state is passed, and shows coachmark after completion", async () => {
		...
		render(
			<AuthProvider>
				<MemoryRouter
					initialEntries={[
						{ pathname: "/branch", state: { justGeneratedProfile: true } },
					]}
				>
					<Routes>
		...
		await waitFor(() => {
			expect(
				screen.getByText("你的自适应学习路径已顺利编织完成。"),
			).toBeTruthy();
		});
		...
		await waitFor(() => {
			expect(
				screen.queryByText("你的自适应学习路径已顺利编织完成。"),
			).toBeNull();
		});
	...
	it("refreshes branch overview when the learning path update event is dispatched", async () => {
		...
		act(() => {
			window.dispatchEvent(
				new CustomEvent("mutiagent-learning-path-updated", {
