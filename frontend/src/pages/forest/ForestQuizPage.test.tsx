...
vi.mock("../../api/forest", () => ({
	fetchForestQuizSession: (...args: unknown[]) =>
		fetchForestQuizSessionMock(...args),
	generateForestQuiz: (...args: unknown[]) => generateForestQuizMock(...args),
	submitForestQuizAttemptStream: (...args: unknown[]) =>
		submitForestQuizAttemptStreamMock(...args),
...
function forestSession(
	overrides: Partial<ForestQuizSession> = {},
): ForestQuizSession {
	return {
		course: {
...
describe("ForestQuizPage", () => {
	...
	it("generates quiz, submits answers, and streams Forest AI analysis", async () => {
		...
		streamForestAiMock.mockImplementation(
			async (_token, _context, _message, onEvent) => {
		...
		await waitFor(() =>
			expect(submitForestQuizAttemptStreamMock).toHaveBeenCalledWith(
				"token-1",
				"quiz_1",
				{ answers: { q1: "A" } },
				expect.any(Function),
		...
		await waitFor(() =>
			expect(screen.getByText("先看题干，再看答案。")).toBeTruthy(),
		);
	});
	...
	it("allows user to send custom follow-up message with handwriting canvas drawing", async () => {
		...
		HTMLCanvasElement.prototype.toDataURL = vi
			.fn()
			.mockReturnValue("data:image/png;base64,drawingdata");

		fetchForestQuizSessionMock.mockResolvedValue(
			forestSession({
				quiz: {
					quiz_id: "quiz_1",
					course_node_id: "year_3_course_2",
		...
		streamForestAiMock.mockImplementation(
			async (_token, _context, message, onEvent) => {
				onEvent({
					event: "forest_ai_text_chunk",
					chunk: `解析自定义问题：${message}`,
				});
				onEvent({ event: "forest_ai_completed", message: "completed" });
			},
		);
		...
		await waitFor(() => {
			expect(
				screen.getByText("解析自定义问题：为什么这里的A是正确答案？"),
			).toBeTruthy();
		});
	});
});
