...
function inferRecoveredHasCompleteProfile(
		...
		async function recoverSession() {
			...
			try {
				...
				if (raw) {
					...
					const cachedUserUid =
						typeof parsed.userUid === "string" ? parsed.userUid : null;
					if (
						...
						cachedHasInFlightAssistant = hasInFlightAssistantSnapshot(
							parsed.messages,
						);
						const explicitHasCompleteProfile =
							typeof parsed.hasCompleteProfile === "boolean"
								? parsed.hasCompleteProfile
								: null;
						const inferredHasCompleteProfile = inferRecoveredHasCompleteProfile(
							parsed.messages,
						);
						cachedHasCompleteProfile =
							inferredHasCompleteProfile ?? explicitHasCompleteProfile;
						if (
							cachedHasCompleteProfile !== null &&
							!cachedHasInFlightAssistant
						) {
							if (cancelled) return;
							recoveredSessionIdRef.current = recoveredSessionId;
							if (recoveryMetaRef) {
								recoveryMetaRef.current = {
									hasCompleteProfile: cachedHasCompleteProfile,
								};
							}
			...
			try {
				const recovered = await fetchSessionRecoveryData(
					token,
					recoveredSessionId,
				);
				if (cancelled) return;
				if (recovered.messages.length === 0 && cachedMessages) {
					...
					if (recoveryMetaRef) {
						recoveryMetaRef.current = {
							hasCompleteProfile: cachedHasCompleteProfile ?? false,
						};
					}
				...
				if (recoveryMetaRef) {
					recoveryMetaRef.current = {
						hasCompleteProfile: recovered.hasCompleteProfile,
					};
				}
