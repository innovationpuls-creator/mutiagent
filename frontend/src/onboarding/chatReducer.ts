...
function updateAssistantById(
	...
	return messages.map((msg, index) =>
		index === messageIndex ? updater(msg) : msg,
	);
}

function mergeRunStep(
	existingStep: AgentRunStep,
	nextStep: AgentRunStep,
): AgentRunStep {
	return {
		...existingStep,
		...nextStep,
		...
		case "STEP": {
			...
			return {
				...
				messages: updateAssistantById(
					...
					(msg) => {
						const trace = [...(msg.runTrace ?? [])];
						const existingIdx = trace.findIndex(
