...
function normalizeSection(value: unknown): LeafSection {
  ...
  if (
    ...
    || !(value.parent_section_id === null || typeof value.parent_section_id === 'string')
...
function normalizeGenerationStatus(value: unknown): LeafGenerationStatus | null {
...
export async function fetchLeafCourse(token: string, courseNodeId: string): Promise<LeafCourseResponse> {
  const response = await fetch(`${API_BASE_URL}/api/leaf/courses/${encodeURIComponent(courseNodeId)}`, {
  ...
  if (!response.ok) {
    ...
    throw new Error((typeof error?.detail === 'string' ? error.detail : null) ?? '叶茂数据加载失败');
  }
  ...
  if (payload.access_state !== 'available' && payload.access_state !== 'locked') {
    throw new Error('叶茂数据格式不正确');
  }
  const composedRaw = isRecord(payload.section_composed_markdowns) ? payload.section_composed_markdowns : {};
  const sectionComposedMarkdowns = Object.fromEntries(
    Object.entries(composedRaw).map(([sectionId, value]) => [sectionId, normalizeComposedSection(value)]),
  );
  ...
  return {
    ...
    first_generatable_chapter_id: typeof payload.first_generatable_chapter_id === 'string' ? payload.first_generatable_chapter_id : null,
