...
describe('streamSession', () => {
  ...
  it('emits session events and returns the completed session turn', async () => {
        ...
        controller.enqueue(
          encoder.encode(
            [
              ...
              'data: {"session_id":"session-1","answer":{"user_message":"请介绍","question_box":{"question":"请介绍","options":[{"label":"基础情况","value":"基础情况","description":"确认基础情况","target_fields":["learning_stage"],"fills":{"learning_stage":"基础情况"}},{"label":"学习目标","value":"学习目标","description":"确认学习目标","target_fields":["short_term_goal"],"fills":{"short_term_goal":"学习目标"}}]}},"agent_trace":[{"step_id":"profile","agent_key":"profile_agent","label":"基础画像智能体","phase":"profile","status":"completed","message":"画像智能体已完成","depends_on":["intent"],"parallel_group":null}],"completed":false,"profile":null,"learning_path":null}',
              '',
            ].join('\n'),
          ),
        );
    ...
    expect(turn.answer.questionBox?.options.map((option) => option.label)).toEqual(['基础情况', '学习目标']);
