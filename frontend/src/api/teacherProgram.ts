...
async function requestProgram<TResponse>(
  ...
  if (!response.ok) {
    ...
    throw new Error((typeof error?.detail === 'string' ? error.detail : null) ?? '人培方案操作失败');
  }
...
export const teacherProgramApi = {
  getTeacherProgram(token: string) {
    return requestProgram<CultivationProgram | null>(token, '/api/teacher/program');
  },
  saveTeacherProgram(token: string, courses: BranchCourseNode[], scope?: ProgramScopePayload) {
    return requestProgram<CultivationProgram>(token, '/api/teacher/program', {
      method: 'PUT',
      body: JSON.stringify(programBody(courses, scope)),
    });
  },
  publishTeacherProgram(token: string, courses: BranchCourseNode[], scope?: ProgramScopePayload) {
    return requestProgram<CultivationProgram>(token, '/api/teacher/program/publish', {
      method: 'POST',
      body: JSON.stringify(programBody(courses, scope)),
    });
  },
  getMatchedProgram(token: string) {
    return requestProgram<CultivationProgram | null>(token, '/api/student/matched-program');
  },
};
