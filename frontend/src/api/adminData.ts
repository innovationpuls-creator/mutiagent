...
export const adminDataApi = {
	...
	programs(token: string) {
		return requestAdminData<CultivationProgram[]>(
			token,
			"/api/admin/data/programs",
		);
	},
	userLearningData(token: string, uid: string) {
		return requestAdminData<UserLearningData>(
			token,
	...
	deleteCohortProgram(
		token: string,
		cohort: Pick<DataCohort, "school" | "major" | "class_name">,
	) {
		...
		return requestAdminData<void>(
			token,
			`/api/admin/data/cohorts/${school}/${major}/${className}/program`,
			{
				method: "DELETE",
			},
		);
