...
function normalizeTimeArrangement(
	value: unknown,
): BranchCourseNode["time_arrangement"] {
	if (!isRecord(value)) {
		return undefined;
	}
...
export function normalizeCourse(value: unknown): BranchCourseNode {
	...
	const isCustom =
		typeof value.is_custom === "boolean" ? value.is_custom : undefined;
	const parentPresetId =
		typeof value.parent_preset_id === "string"
			? value.parent_preset_id
			: undefined;
	...
	const keyPoints = isStringArray(value.key_points)
		? value.key_points
		: undefined;
	const difficultPoints = isStringArray(value.difficult_points)
		? value.difficult_points
		: undefined;
	const acceptanceCriteria = isStringArray(value.acceptance_criteria)
	...
	for (const [key, value] of Object.entries(raw as Record<string, unknown>)) {
		if (isRecord(value)) {
			result[key] = {
				...
				difficulty_fit:
					typeof value.difficulty_fit === "number" ? value.difficulty_fit : 0,
				completeness:
					typeof value.completeness === "number" ? value.completeness : 0,
...
export async function fetchBranchOverview(
	...
	const years = Object.fromEntries(
		Object.entries(payload.years).map(([gradeId, year]) => [
			gradeId,
			normalizeYear(year),
		]),
	);
...
export async function fetchCanopyOverview(
	token: string,
): Promise<CanopyOverview> {
