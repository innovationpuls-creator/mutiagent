...
function isNullableUnknownRecord(
	value: unknown,
): value is UnknownRecord | null {
...
function normalizeChatStartResponse(value: unknown): ChatStartResponse {
	...
	if (
		...
		!(
			value.reply_text === null ||
			value.reply_text === undefined ||
...
function pickLearningPath(
	...
	const validPaths =
		Object.values(yearLearningPaths).filter(isLearningPathResult);
...
function pickCourseKnowledge(
	courseKnowledge: Record<string, unknown> | null,
): CourseKnowledgeResult | null {
...
function pickProfile(
	...
	if (profile.type !== "basic_profile" && profile.type !== "collecting")
		return null;
	if (
		profile.type === "basic_profile" &&
		!hasCompleteBasicProfileRecord(profile, REQUIRED_PROFILE_KEYS)
	) {
		return null;
	}
...
function pickProfileSummaryText(
	profile: Record<string, unknown> | null,
): string | null {
	if (!isUnknownRecord(profile)) return null;
...
export function isLearningPathStructuredCompletionContent(
	content: string,
): boolean {
	return (
...
export function isCourseKnowledgeStructuredCompletionContent(
	content: string,
): boolean {
	return (
...
function parsePersistedMessages(
	...
	const baseTimestamp = Number.isFinite(parsedUpdatedAt)
		? parsedUpdatedAt
		: Date.now();
...
function buildSessionStructuredData(
...
function attachStructuredDataToRecoveredMessages(
	...
	const profileTargetIndex = structuredData.profile
		? findLastMatchingAssistantMessageIndex(
				messages,
				(content) =>
...
function normalizeSessionEvent(
	...
	const event = (
		rawEvent === "message" ? getString(payload.type) : rawEvent
	) as SessionEventName;
	...
	return {
		...
		retryAction:
			getString(payload.retryAction) === "retry_learning_path"
				? "retry_learning_path"
				: undefined,
...
export async function startSession(
	...
	const payload = normalizeChatStartResponse(
		await requestChat<unknown>("/api/chat/start", token, { query }),
	);
...
export async function streamSession(
	...
	if (!activeSessionId) {
		const startPayload = normalizeChatStartResponse(
			await requestChat<unknown>("/api/chat/start", token, { query }),
		);
