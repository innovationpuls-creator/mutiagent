...
function TestComponent() {
  const { widgetState, setWidgetState, pendingMessage, openWithMessage, clearPendingMessage } = useAiWidget();
  return (
    <div>
      ...
      <button onClick={() => openWithMessage('开始第一门课')}>Open With Message</button>
...
function PendingMessageIdProbe() {
  ...
  return (
    <div>
      ...
      <button onClick={() => openWithMessage('第一条消息')}>First Pending</button>
      <button onClick={() => openWithMessage('第二条消息')}>Second Pending</button>
    </div>
  );
}
