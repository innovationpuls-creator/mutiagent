...
function isCurrentLearningProgressState(
	value: unknown,
): value is CurrentLearningCourse["progress_state"] {
	return value === "in_progress" || value === "completed";
}

export function isLearningPathResult(
	value: unknown,
): value is LearningPathResult {
	...
	if (
		!hasRecord(learningGoal) ||
		!hasRecord(learnerBaseline) ||
		!hasRecord(gradePlans)
	)
		return false;
	if (
		!hasRecord(knowledgeGraph) ||
		!hasRecord(resourceContract) ||
		!hasRecord(updateContract)
	)
		return false;
	...
	const isGradePlan = (gradePlan: unknown): gradePlan is GradePlanRecord =>
		hasRecord(gradePlan) &&
	...
	const hasCurrentGradePlan =
		hasString(currentGradeId) && hasGradePlan(currentGradeId);
	const currentGradePlan = hasString(currentGradeId)
		? gradePlans[currentGradeId]
		: undefined;
	const currentGradeCourseNodes =
		hasCurrentGradePlan && isGradePlan(currentGradePlan)
			? currentGradePlan.course_nodes
			: [];

	const hasCurrentCourseNode =
		currentGradeCourseNodes.length === 0 ||
		currentGradeCourseNodes.some(
			(courseNode) =>
				hasRecord(courseNode) &&
				courseNode.course_node_id === currentLearningCourse.course_node_id,
		);

	const hasValidGradePlanKeys = Object.keys(gradePlans).every(
		(gradeId) =>
			gradeId === "year_1" ||
			gradeId === "year_2" ||
			gradeId === "year_3" ||
	...
	const hasValidGradePlanIds = Object.entries(gradePlans).every(
		([gradeId, gradePlan]) => {
	...
	const hasTimeArrangement = (value: unknown) => {
		if (!hasRecord(value)) return false;
		return (
			hasString(value.semester_scope) &&
	...
	const hasCourseNodeShape = (courseNode: unknown) => {
		...
		return (
			hasString(courseNode.course_node_id) &&
	...
	const hasValidCourseNodes = Object.entries(gradePlans).every(
		([gradeId, gradePlan]) => {
			if (!isGradePlan(gradePlan)) return false;
			return gradePlan.course_nodes.every(
				(courseNode) =>
	...
	return (
		hasString(learningGoal.target_course_or_skill) &&
...
export function isCourseKnowledgeResult(
	value: unknown,
): value is CourseKnowledgeResult {
	if (!hasRecord(value)) return false;
		...
		value.sections.every(
			(section) =>
				...
				(section.parent_section_id === null ||
					hasString(section.parent_section_id)) &&
